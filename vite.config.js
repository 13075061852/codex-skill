...
function deepseekPlugin(env) {
  const apiPath = "/api/deepseek-analyze";
  const deepseekMiddleware = async (request, response) => {
    ...
    try {
      ...
      if (!projectIdea) {
        sendJson(response, 400, { error: "项目方向不能为空。" });
        return;
      }
      ...
      sendJson(response, 200, { analysis: extractJsonObject(content) });
    } catch (error) {
  ...
  return {
    name: "codex-skill-deepseek-proxy",
    configureServer(server) {
      server.middlewares.use((request, response, next) => {
        ...
        deepseekMiddleware(request, response);
      });
    },
...
export default defineConfig(({ mode }) => {
  ...
  return {
    plugins: [react(), deepseekPlugin(env)],
  };
});
